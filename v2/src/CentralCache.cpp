...
(size_t index, size_t currentCount, std::chrono::steady_clock::time_point currentTime)
{
    // 基于计数和时间的双重检查
...
void CentralCache::updateSpanFreeCount(SpanTracker* tracker, size_t newFreeBlocks, size_t index)
{
    ...
    if (newFreeCount == tracker->blockCount.load(std::memory_order_relaxed))
    {
        ...
        while (curr)
        {
            ...
            if (curr >= spanAddr && curr < static_cast<char*>(spanAddr) + numPages * PageCache::PAGE_SIZE)
            {
                if (prev)
                {
...
void* CentralCache::fetchFromPageCache(size_t size)
{
    ...
    // 2. 根据大小决定分配策略
    if (size <= PageCache::PAGE_SIZE * SPAN_PAGES)
    {
        // 小于等于32KB的请求，使用固定8页
